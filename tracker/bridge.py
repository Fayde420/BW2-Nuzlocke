...
# ── Konfiguration ──
# state.json-Ort universell (wie live_team.lua): Umgebungsvariable
# AUTOTRACKER_DIR, sonst neben diesem Skript. So muss nie ein Pfad angepasst
# werden — Lua schreibt und diese Bridge liest denselben Ort.
_ATDIR         = os.environ.get("AUTOTRACKER_DIR")
STATE_FILE     = (Path(_ATDIR) if _ATDIR else Path(__file__).resolve().parent) / "state.json"
